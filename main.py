...
def update_doctor_dropdown():
    ...
    # Add all doctors to the dropdown
    for doctor in doctors:
...
def on_clinic_patient_change(*args):
    ...
    for selected_doctor in selected_doctors_from_clinic:
        doctor_patient_menu['menu'].add_command(label=selected_doctor.get_doctor_id(),
                                                command=lambda c=selected_doctor.get_doctor_id(): doctor_patient_var.set(c))
...
# Frames - new column layout with equal proportions
...
# Layout - adjusted to place details at the top
...
# Configure grid weights for optimal distribution
...
# Configure column weights for details frame
for i in range(6):
    ramka_szczegoly_obiektow.grid_columnconfigure(i, weight=1)

# ramka_clinics (Clinic Management)
...
button_clinic_map_all = Button(button_frame_clinics, text="Generuj mapę wszystkich placówek", command=generate_map_of_clinics,
                              font=("Arial", 9), bg="#2196F3", fg="white")
button_clinic_map_all.pack(side=LEFT, padx=5)

button_clinic_map_employees = Button(button_frame_clinics, text="Mapa lokalizacji pracowników placówki", command=employee_localization_handler,
                                    font=("Arial", 9), bg="#2196F3", fg="white")
...
# ramka_doctors (Doctor Management)
...
# Label and Entry for Doctor's Name
...
# Label and Entry for Doctor's Surname
...
# Label and Entry for Doctor's Location
...
# Label and OptionMenu for Clinic
...
clinic_patient_var.set("")  # Default value
...
# Etykieta i pole dla lokalizacji
...
map_widget.set_position(52.0, 19.5)  # Centered on Poland
